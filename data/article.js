const article = {
  id: 'article',
  name: '文章',
  lists: [
    {
      name: 'Five common mistakes writing react components',
      href: 'https://www.lorenzweiss.de/common_mistakes_react_hooks/',
    },
    {
      name: 'React Libraries in 2020',
      href: 'https://www.robinwieruch.de/react-libraries#writing-documentation-with-react',
    },
    {
      name: 'vue3 News',
      href: 'https://github.com/vue3/vue3-News',
    },
    {
      name: '提問的智慧',
      href: 'https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way',
    },
    {
      name: 'branch 同步 master',
      href: 'https://www.itread01.com/content/1547544259.html'
    },
    {
      name: 'Webpack Alias',
      href: 'https://medium.com/itsoktomakemistakes/webpack-alias-%E8%AE%93%E9%96%8B%E7%99%BC%E6%9B%B4%E6%96%B9%E4%BE%BF-ff84dca0cb47'
    },
    {
      name: 'MAC ITERM2',
      herf: 'https://dustinhsiao21.com/2019/04/09/'
    },
    {
      name: 'VS Code 23個常用快捷鍵速查表',
      href: 'https://hackmd.io/@sean666/HJ3x8K6Z8'
    },
    {
      name: 'webpack01',
      href: 'https://magiclen.org/webpack/'
    },
    {
      name: '配置 Typescript、ESLint',
      href: 'https://juejin.im/post/5d06f2d36fb9a07eeb13a778'
    },
    {
      name: 'StackOverflow Book',
      href: 'https://books.goalkicker.com/'
    },
    {
      name: 'clientX,offsetX,screenX,pageX',
      href: 'https://kknews.cc/zh-tw/news/r3pzzr.html'
    },
    {
      name: 'Vue 面試題',
      href: 'https://github.com/sudheerj/vuejs-interview-questions-chinese'
    },
    {
      name: 'Vue 實作模式',
      href: 'https://github.com/yoyoys/vue-patterns-cht'
    },
    {
      name: 'Git branch model',
      href: 'https://blog.wu-boy.com/2011/03/git-%E7%89%88%E6%9C%AC%E6%8E%A7%E5%88%B6-branch-model-%E5%88%86%E6%94%AF%E6%A8%A1%E7%B5%84%E5%9F%BA%E6%9C%AC%E4%BB%8B%E7%B4%B9/'
    },
    {
      name: 'Composition api',
      href:
        'https://css-tricks.com/an-early-look-at-the-vue-3-composition-api-in-the-wild/'
    },
    {
      name: 'Markdown Cheatsheet 中文版',
      href: 'https://gist.github.com/billy3321/1001749662c370887c63bb30f26c9e6e'
    },
    {
      name: 'Javascript Reduce Functions',
      href: 'https://www.freecodecamp.org/news/10-js-util-functions-in-reduce/'
    },
    {
      name: 'ICSS',
      href: 'https://github.com/chokcoco/iCSS'
    },
    {
      name: 'Debounce 和 Throttle',
      href: 'https://mropengate.blogspot.com/2017/12/dom-debounce-throttle.html'
    },
    {
      name: 'Web Worker',
      href: 'https://www.html5rocks.com/zh/tutorials/workers/basics/'
    },
    {
      name: 'utility library List',
      href: 'https://css-tricks.com/need-css-utility-library/'
    },
    {
      name: 'Vue Directives Useful',
      href:
        'https://www.telerik.com/blogs/15-must-have-vue-directives-that-will-significantly-maximize-your-productivity'
    },
    {
      name: 'NPM 發布 2',
      href:
        'https://medium.com/@debbyji/%E6%88%91%E7%9A%84%E7%AC%AC%E4%B8%80%E5%80%8B-npm-%E5%A5%97%E4%BB%B6-%E6%8A%8A-vue-component-%E6%89%93%E5%8C%85%E5%88%B0-npm-%E5%90%A7-e5f9a6901c5c'
    },
    {
      name: 'NPM 發布 1',
      href: 'https://segmentfault.com/a/1190000017461666'
    },
    {
      name: 'Will TypeScript',
      href: 'https://willh.gitbook.io/typescript-tutorial/'
    },
    {
      name: 'jQuery to JavaScritp',
      href:
        'https://tobiasahlin.com/blog/move-from-jquery-to-vanilla-javascript/'
    },
    {
      name: '30天精通 Git 版本控管',
      href:
        'https://github.com/doggy8088/Learn-Git-in-30-days/blob/master/zh-tw/README.md'
    },
    { name: 'sass中文檔', href: 'https://www.css88.com/doc/sass' },
    { name: 'SCSS 速查表', href: 'http://caibaojian.com/scb/sass.html' },
    {
      name: 'CSS Grid 大全',
      href: 'https://css-tricks.com/snippets/css/complete-guide-grid/'
    },
    {
      name: 'CSS保哥翻譯技巧',
      href:
        'https://github.com/AllThingsSmitty/css-protips/blob/master/translations/zh-TW/README.md'
    },
    {
      name: 'Vue 2.6.6 文章',
      href:
        'https://dev.to/f3ltron/vue-266-release-part1-23b4?fbclid=IwAR2z8lg0EDrSUtDs-i-UBKA0PpzUbOyV8B91hIEioIo1-vn0oS1AGCU4Pm8'
    },
    {
      name: '正規式整理',
      href: 'https://kiey0930.pixnet.net/blog/post/141007526'
    },
    {
      name: 'JS陣列操作大全',
      href:
        'https://medium.freecodecamp.org/15-useful-javascript-examples-of-map-reduce-and-filter-74cbbb5e0a1f'
    }
  ]
};
